...
try 5 'int plus(x, y){return x + y;} int main(){ return plus(3,2);}'
try 10 'int main(){int x; int i; x = 0; for(i = 0; i < 10; i+=1){x+=1;} return x;}'
try 55 'int sum(m, n) { int acc; int m; int i;acc = 0; for (i = m; i < n; i = i + 1) acc = acc + i; return acc;} int main() {return sum(0, 11);}'
try 30 'int rec(n){if(n == 1){return 30;} return rec(1);} int main(){return rec(2);}'
try 4 'int bar(){return 2;} int main(){return bar() + bar();}'
try 30 'int bar(n){return n;} int main(){return bar(10) + bar(20);}'
try 30 'int bar(n){return n;} int main(){return bar(5+5) + bar(40/2);}'
try 18 'int bar(n){return n;} int main(){int a; a = 5; return bar(a+a) + bar(40/a);}'
try 55 'int fib(n){ if(n == 1 || n == 2) {return 1;} else {return fib(n-1) + fib(n-2);}} int main(){ return fib(10); }'
try 89 'int fib(n){ if(n == 1 || n == 2) {return 1;} else {return fib(n-1) + fib(n-2);}} int main(){ return fib(11); }'
try 89 'int fib(n){ if(n == 1 || n == 2) {return 1;} return fib(n-1) + fib(n-2);} int main(){ return fib(11); }'
